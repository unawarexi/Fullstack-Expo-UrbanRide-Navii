...
import { uploadDocument, uploadImage } from "@/lib/imagekit";
...
const AdminForm = () => {
  ...
  const handleAdminLogin = async (data: AdminLoginData) => {
    ...
    try {
      // Here you would validate admin credentials with your API
      ...
      const isValidAdmin = await validateAdminCredentials(data);

      if (isValidAdmin) {
        // Here, send verificationCardUrl along with other data to your API
        // Example:
        // await adminApi.registerAdmin({ ...data, verificationCardUrl });
        ...
        showToast("Invalid admin credentials. Please check your details.", "error");
      }
    } catch (err: any) {
  ...
  const validateAdminCredentials = async (data: AdminLoginData): Promise<boolean> => {
    // This would be replaced with actual API validation
    // For demo purposes, we'll simulate validation
    return data.employeeId && data.email.includes("@");
  };
